...
export default function TeamAdminPage({ initialItems, mediaItems }) {
  ...
  return (
    <>
      ...
      <MediaLibraryPickerModal
        key={`team-media-picker-${isMediaPickerOpen ? "open" : "closed"}-${draft.image || "empty"}`}
        ...
        onSelect={(item) => {
          if (item?.url) {
            ...
            setFeedback({ type: "success", message: "Profile photo selected from media library." });
          }
        ...
        title="Choose Team Profile Photo"
        description="Reuse an uploaded image for this profile instead of adding a duplicate."
      />
    </>
  );
}
