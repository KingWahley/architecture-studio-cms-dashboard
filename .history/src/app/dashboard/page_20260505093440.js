...
import { Briefcase, Building2, Calendar, Clock3, MessageSquare, Plus } from "lucide-react";
...
export default async function DashboardPage() {
  const { stats, quickActions, recentMessages, upcomingAppointments } = await getDashboardData();
          ...
          <Card className="h-full border-border-subtle/50 shadow-sm">
            ...
            <CardContent className="space-y-4">
              {upcomingAppointments.map((appointment) => (
                <div key={appointment.id} className="rounded-2xl border border-border-subtle bg-surface-alt/75 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-display text-base font-semibold text-on-surface">{appointment.client}</p>
                      <p className="mt-1 text-sm text-text-secondary">{appointment.topic}</p>
                    </div>
                    <StatusBadge variant={appointment.status}>{appointment.status}</StatusBadge>
                  </div>

                  <div className="mt-4 space-y-2 text-sm text-text-secondary">
                    <div className="flex items-center gap-2">
                      <Calendar size={14} className="text-accent-deep-blue" />
                      <span>{appointment.date} . {appointment.time}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <Clock3 size={14} className="text-accent-deep-blue" />
                      <span>{appointment.location}</span>
                    </div>
                  </div>

                  <p className="mt-3 text-sm leading-6 text-text-secondary">{appointment.notes}</p>
                </div>
              ))}

              {upcomingAppointments.length === 0 && (
                <div className="py-12 text-center">
                  <p className="text-sm text-text-secondary">No upcoming appointments to display.</p>
                </div>
              )}
            </CardContent>
          </Card>
