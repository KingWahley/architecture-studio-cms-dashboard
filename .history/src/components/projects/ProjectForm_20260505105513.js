...
export default function ProjectForm({ project, categories }) {
  ...
  const addGalleryUrl = () => {
    setDraft((current) => ({
      ...
      gallery: [
        ...current.gallery,
        { id: `gallery-${crypto.randomUUID()}`, url: "", alt: "" },
      ],
    }));
  };
              ...
              <div className="grid gap-5 md:grid-cols-2">
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-text-secondary">
                    Categories are managed from the <Link href="/projects" className="text-accent-deep-blue hover:underline">Projects list</Link>.
                  </p>
                </div>
          ...
          <Card className="overflow-hidden">
            <CardHeader className="border-b border-border-subtle bg-surface-main/80">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <CardDescription>Upload multiple images, edit alt text, and drag to reorder.</CardDescription>
                </div>
                <div className="flex gap-3">
                  <Button type="button" variant="secondary" onClick={addGalleryUrl} className="gap-2">
                    <ImagePlus size={16} />
                    Add URL
                  </Button>
                  <Button
            ...
            <CardContent className="space-y-4 p-6">
              {draft.gallery.length === 0 ? (
                <div className="rounded-2xl border border-dashed border-border-subtle bg-surface-alt/60 px-5 py-8 text-center text-sm text-text-secondary">
                  No gallery items yet. Upload images or add external image URLs.
                </div>
              ) : null}
                ...
                >
                  ...
                  <div className="mt-4 space-y-4">
                    <div className="space-y-2">
                      ...
                      <Input value={item.url} onChange={(event) => updateGalleryItem(item.id, "url", event.target.value)} placeholder="https://example.com/project-image.jpg" className="h-11" />
                    </div>
